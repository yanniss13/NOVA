...
- **Commande Discord `/jarvis`** : l'assistant IA de la confrérie, qui se
  ...
  **Lot 2c (objets)** : `outils/fabrication/extraire-objets.js` écrit
  ...
  le groupe × part de la ligne. Paquet aléatoire (`DropPack_Type` vrai) :
  un seul objet, les `Rate` sont des **poids** ramenés à leur somme
  (Belette : 80 %). Sinon `Rate`/10000 (Banakro : 1,5 % → 10 %). Vérifié
  contre 7dsorigin.app ; aucune chance quand un nom couvre deux lignes au
  même niveau de monde. Étape 3 : les
