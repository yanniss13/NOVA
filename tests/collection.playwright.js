...
(async()=>{
  ...
  try{
    ...
    /* 238 avant la version 2.0 : le Nunchaku de l'âme vorace, l'arme de Ban,
       porte le compte à 239. */
    assert.equal(total, 239, "les armes et les armures gravées du dépôt");
    ...
    assert.match(await progression(), /3 \/ 239 possédés — 236 à trouver/);
    ...
    assert.match(await progression(), /4 \/ 239 possédés — 235 à trouver/);
    ...
    await page.waitForFunction(() =>
      ...
      && document.querySelectorAll("#collectionBody .wiki-tile").length < 239
    );
    ...
    assert.match(await progression(), /1 \/ 239 possédés — 238 à trouver/);
    ...
    await page.waitForFunction(() => {
      ...
      return tuilesVues.length > 0 && tuilesVues.length < 239;
    });
