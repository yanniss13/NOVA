...
function fakeServiceWorker({ withController, withWaiting }){
  return `(() => {
    const state = {
      messages: [],
      updateCalls: 0,
    ...
    const listeners = { updatefound: [], controllerchange: [] };
    const emit = (type, event) => {
      listeners[type].slice().forEach(listener => listener(event || { type }));
    };
    ...
    const makeWorker = workerState => {
      ...
      const worker = {
        id: ++workerId,
        state: workerState,
        postMessage(data){ state.messages.push(data); },
    ...
    const container = {
      ...
      removeEventListener(type, listener){
        registration.removeEventListener(type, listener);
      },
