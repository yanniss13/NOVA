...
La saisie manuelle d'un héros complet représente huit pièces, chacune avec un
niveau, un renforcement et ses enchantements. C'est le principal frein au
...
- **l'arme**. Sa configuration porte quatre dimensions (`gradeGameId`, `level`,
  `promotion`, `overlimit`) là où une armure n'en porte que deux. L'inversion
  décrite plus bas est mesurée sur deux dimensions ; sur quatre son unicité est
  douteuse et n'a pas été vérifiée. L'arme reste éditable à la main ;
